...
public class JsonFilterConfiguration implements FilterConfiguration {
  ...
  private JsonFilterConfiguration(
   ...
   */
  public static JsonFilterConfiguration loadFromJsonResource(String resourcePath)
      throws IOException {
    ...
    List<String> includePatterns =
        mapper.convertValue(
            rootNode.get("includePatterns"),
            mapper.getTypeFactory().constructCollectionType(List.class, String.class));
    List<String> excludePatterns =
        mapper.convertValue(
            rootNode.get("excludePatterns"),
            mapper.getTypeFactory().constructCollectionType(List.class, String.class));
    Set<String> blacklistedExtensions =
        mapper.convertValue(
            rootNode.get("blacklistedExtensions"),
    ...
    includePatterns.stream()
        .limit(3)
        .forEach(pattern -> LOGGER.debug("  '{}' (Length: {})", pattern, pattern.length()));
    ...
    blacklistedExtensions.stream()
        .limit(10)
        .forEach(ext -> LOGGER.debug("  '{}' (Length: {})", ext, ext.length()));
    // --- /ОТЛАДКА ---
