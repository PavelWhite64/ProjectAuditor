  ...
  public void generate(AnalysisResult result, String outputFile) {
    try (PrintWriter writer =
      ...
      for (int i = 0; i < files.size(); i++) {
        ...
        writer.write(
            "      \"path\": \"" + ReportUtils.escapeJson(file.getRelativePath()) + "\",\n");
        writer.write("      \"sizeKB\": " + (file.getLength() / 1024) + ",\n");
        writer.write(
            "      \"language\": \"" + ReportUtils.escapeJson(file.getExtension()) + "\",\n");
        writer.write(
            "      \"icon\": \""
                + fileIconService.getIcon(file.getExtension())
