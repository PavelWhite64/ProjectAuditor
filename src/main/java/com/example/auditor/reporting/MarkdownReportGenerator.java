...
public class MarkdownReportGenerator {
  ...
  public void generate(
    ...
    try (BufferedWriter writer =
      ...
      writer.write(
          "**Ограничения:** Макс. размер содержимого: "
        ...
        for (FileInfo file : files) {
          ...
          writer.write(
              "\n"
                  ...
                  + icon
                  + " "
                  + escapeMarkdown(file.getRelativePath())
                  + " (`"
                  + String.format(Locale.US, "%.1f", kb)
                  + " KB`)\n");
      ...
      writer.write(
          "  > Проект **"
              + ReportUtils.escapeMarkdown(projectName)
              + "** готов для анализа LLM.\n");
