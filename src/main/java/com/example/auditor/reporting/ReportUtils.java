...
public class ReportUtils {
  ...
  public static String readFileContent(
    ...
    if (!isPathInsideBaseDirectory(filePath, baseDirectoryPath)) {
      LOGGER.warn(
          "Попытка чтения файла за пределами базовой директории: {}. Файл будет пропущен.",
          filePath);
  ...
  private static String readFileWithLineLimit(Path filePath, int maxLines, FileSystem fileSystem)
      throws IOException {
      ...
      if (line != null) {
        content.append(
            String.format(
                ...
                lineCount, maxLines));
        LOGGER.debug(
            "Файл обрезан: {} (прочитано {} строк из лимита {})", filePath, lineCount, maxLines);
      }
    }
  ...
  public static String generateTreeMarkdown(List<FileInfo> files, FileIconService fileIconService) {
    ...
    List<String> sortedPaths =
        files.stream().map(FileInfo::getRelativePath).sorted().collect(Collectors.toList());
